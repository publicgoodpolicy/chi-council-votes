...
(function (root, factory) {
  ...
  function methodologyView(verify, office, duesExcluded) {
    ...
    return METH_FRAME_OPEN +
      ...
      'deadline.</p>' +
      // SB-METH-1 (R46): the school-board view's one ratified string, composed exactly as the
      // council branch composes C5 — same literal, bare values, whole or not at all. It replaces
      // the runtime pull-date clause this branch used to carry, which was in no register and
      // bound only the pull date. [METH/REGISTER] compares this paragraph to RULINGS.md §SB-METH-1.
      ((verify && verify.dataThrough != null && verify.pulled != null)
        ? ('<p>Contribution data is current through ' + verify.dataThrough + ', from the Illinois ' +
           'State Board of Elections bulk export of ' + verify.pulled + '.</p>')
        : '') +
